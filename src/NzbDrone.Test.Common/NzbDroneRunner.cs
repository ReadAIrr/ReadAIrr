...
namespace NzbDrone.Test.Common
{
    public class NzbDroneRunner
    {
        ...
        public NzbDroneRunner(Logger logger, PostgresOptions postgresOptions, int port = 8246)
        {
        ...
        public void Kill()
        {
            try
            {
                if (_nzbDroneProcess != null)
                {
                    ...
                    if (_nzbDroneProcess.HasExited)
                    {
                        var log = File.ReadAllLines(Path.Combine(AppData, "logs", "readairr.trace.txt"));
