        ...
        private ValidationFailure TestConnection()
        {
            try
            {
                ...
                else if (Settings.MusicCategory.IsNullOrWhiteSpace())
                {
                    ...
                    {
                        IsWarning = true,
                        DetailedDescription = "ReadAIrr will not attempt to import completed downloads without a category."
                    };
                }
                ...
                if (RemovesCompletedDownloads(config))
                {
                    ...
                    {
                        DetailedDescription = "ReadAIrr will be unable to perform Completed Download Handling as configured. You can fix this in qBittorrent ('Tools -> Options...' in the menu) by changing 'Options -> BitTorrent -> Share Ratio Limiting' from 'Remove them' to 'Pause them'."
                    };
                }
            }
        ...
        private ValidationFailure TestCategory()
        {
            ...
            if (Settings.MusicCategory.IsNotNullOrWhiteSpace() && !labels.ContainsKey(Settings.MusicCategory))
            {
                ...
                if (!labels.ContainsKey(Settings.MusicCategory))
                {
                    ...
                    {
                        DetailedDescription = "ReadAIrr was unable to add the label to qBittorrent."
                    };
                }
            }
